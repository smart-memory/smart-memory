...
Patches must target the imported names in smartmemory_cc.cli (not the source module),
since cli.py uses `from smartmemory_cc.storage import ingest, recall` — the names
...
def test_persist_cmd(runner):
    ...
    with patch("smartmemory_cc.cli.ingest", return_value="item-abc") as mock_ingest:
        from smartmemory_cc.cli import cli
...
def test_ingest_cmd(runner):
    ...
    with patch("smartmemory_cc.cli.ingest", return_value="item-xyz") as mock_ingest:
        from smartmemory_cc.cli import cli
...
def test_recall_cmd(runner):
    ...
    with patch(
        "smartmemory_cc.cli.recall", return_value="## SmartMemory Context\n- hello"
    ) as mock_recall:
        from smartmemory_cc.cli import cli
...
def test_recall_cmd_no_cwd(runner):
    ...
    with patch("smartmemory_cc.cli.recall", return_value="") as mock_recall:
        from smartmemory_cc.cli import cli
