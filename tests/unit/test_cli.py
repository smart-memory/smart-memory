...
Patches must target the imported names in smartmemory_app.cli (not the source module),
since cli.py uses `from smartmemory_app.storage import ingest, recall` — the names
...
def test_persist_cmd(runner):
    ...
    with patch("smartmemory_app.cli.ingest", return_value="item-abc") as mock_ingest:
...
def test_ingest_cmd(runner):
    ...
    with patch("smartmemory_app.cli.ingest", return_value="item-xyz") as mock_ingest:
        from smartmemory_app.cli import cli
...
def test_recall_cmd(runner):
    ...
    with patch(
        "smartmemory_app.cli.recall", return_value="## SmartMemory Context\n- hello"
    ) as mock_recall:
        from smartmemory_app.cli import cli
...
def test_recall_cmd_no_cwd(runner):
    ...
    with patch("smartmemory_app.cli.recall", return_value="") as mock_recall:
        from smartmemory_app.cli import cli
