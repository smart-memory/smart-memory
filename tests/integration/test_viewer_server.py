...
class TestImportNoSideEffects:
    def test_app_importable_without_uvicorn(self):
        ...
        from smartmemory_cc.viewer_server import app  # noqa: F401
        assert app is not None

    def test_app_is_fastapi_instance(self):
        from fastapi import FastAPI
        from smartmemory_cc.viewer_server import app
...
def viewer_client(monkeypatch):
    ...
    import smartmemory_cc.local_api as _api_mod
...
class TestMainCallsStartBackground:
    def test_main_uses_start_background_not_raw_thread(self):
        ...
        from smartmemory_cc import viewer_server

        # Patch out uvicorn.run so main() returns immediately
        with (
            patch("smartmemory_cc.viewer_server.uvicorn") as mock_uvicorn,
            patch("smartmemory_cc.events_server.start_background") as mock_start_bg,
        ):
    ...
    def test_main_does_not_open_browser_when_disabled(self):
        ...
        with (
            patch("smartmemory_cc.viewer_server.uvicorn") as mock_uvicorn,
            patch("smartmemory_cc.events_server.start_background"),
            patch("smartmemory_cc.viewer_server.webbrowser") as mock_browser,
            patch("smartmemory_cc.viewer_server.threading") as mock_threading,
        ):
            mock_uvicorn.run = MagicMock()
            viewer_server.main(port=19005, open_browser=False)
