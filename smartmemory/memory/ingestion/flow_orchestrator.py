        ...
        if isinstance(add_result, dict):
            ...
            for i, entity in enumerate(entities):
                if not isinstance(entity, MemoryItem):
                    raise TypeError(f"Expected MemoryItem object at entity index {i}, got {type(entity)}")
                if not getattr(entity, 'metadata', None) or 'name' not in entity.metadata:
                    raise ValueError(f"MemoryItem entity at index {i} missing required 'name' in metadata")
